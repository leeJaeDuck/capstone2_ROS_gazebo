...
def set_picture():
    ...
    send_service(dog_state_msg)
    send_service(cat_state_msg)


def current_ball_cb(data):
    global picture1, picture2, picture3, current_picture

    if data == 1: # completed first ball
        current_picture = picture2
    elif data == 2: # completed second ball
...
def main():
    ...
    rospy.init_node('animal_node', anonymous=True)
